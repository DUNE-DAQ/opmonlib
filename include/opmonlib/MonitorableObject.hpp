...
ERS_DECLARE_ISSUE(opmonlib, EntryWithNoData, "OpMonEntry of type " << type << " has no data", ((std::string)type))
...
using OpMonLevel =
  std::invoke_result<decltype(&dunedaq::confmodel::OpMonConf::get_level), dunedaq::confmodel::OpMonConf>::type;
...
public:
  ...
  static bool publishable_metric(OpMonLevel entry, OpMonLevel system) noexcept { return (entry < system); }
...
private:
  ...
  using const_metric_counter_t =
    std::invoke_result<decltype(&dunedaq::opmonlib::opmon::MonitoringTreeInfo::n_published_measurements),
  ...
  using const_time_counter_t =
    std::invoke_result<decltype(&dunedaq::opmonlib::opmon::MonitoringTreeInfo::cpu_elapsed_time_us),
                       dunedaq::opmonlib::opmon::MonitoringTreeInfo>::type;
