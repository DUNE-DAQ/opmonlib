...
ERS_DECLARE_ISSUE(opmonlib, MonitoringThreadNotSet, "Monitoring thread not set", ERS_EMPTY)

ERS_DECLARE_ISSUE(opmonlib, FailedMonitoringThread, "Monitoring thread failed to start", ERS_EMPTY)

ERS_DECLARE_ISSUE(opmonlib, MissingConfiguration, "Confiration is not set", ERS_EMPTY)
}
...
public:
   ...
   */
  explicit OpMonManager(std::string session, std::string name, std::string opmon_facility_uri = "stdout")
