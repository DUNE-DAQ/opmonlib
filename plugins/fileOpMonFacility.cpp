...
fileOpMonFacility::~fileOpMonFacility()
{
  ...
  auto ret = m_writing_variable.wait_for(lock, max_delay, [&]() { return m_writing_counter.load() == 0; });
  if (!ret) {
    ers::error(FileClosedBeforeWritingComplete(ERS_HERE, max_delay.count(), m_writing_counter.load()));
  }
}

void
fileOpMonFacility::publish(opmon::OpMonEntry&& e) const
{

  if (m_stop_request.load()) {
    throw OpMonPublishFailure(
      ERS_HERE, get_URI(), e.measurement(), to_string(e.origin()), FacilityStopRequested(ERS_HERE));
  }

  ++m_writing_counter;
  std::thread(&fileOpMonFacility::write, this, std::move(e)).detach();
}

void
fileOpMonFacility::write(opmon::OpMonEntry&& e) const noexcept
{
  std::string json;
  ...
  try {
    ...
    ers::error(OpMonPublishFailure(
      ERS_HERE, get_URI(), e.measurement(), to_string(e.origin()), WritingFailed(ERS_HERE, json, except)));
  }
