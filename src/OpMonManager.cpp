...
OpMonManager::OpMonManager(std::string session, std::string name, facility_ptr_t f_ptr)
...
OpMonManager::start_monitoring()
{
  ...
  TLOG() << "Starting a new monitoring thread with interval " << m_cfg.load()->get_interval().count()
         << " seconds, at level " << get_opmon_level();
...
void
OpMonManager::run(std::stop_token stoken)
{
  ...
  while (!stoken.stop_requested()) {
    ...
    auto time_span =
      std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last_collection_time);
